...
# Plot sediment-associated biofilms
plot_SB_phylum_18 <- ggplot(df_SB_18, aes(x = Conditions, y = Relative_Abundance, fill = Phylum)) +
  ...
  theme_test(base_family = "Arial") +
  labs(title = "Sediment-associated biofilms", x = "", y = "Relative abundance (%)") +
...
ggplot(df_WSB_18_pc, aes(x = "", y = mean, fill = Phylum)) +
  ...
  geom_label_repel(aes(label = paste0(round(mean, 1), "%"), y = pos),
...
plot_hl <- ggplot(File_HL_data, aes(x = minutes, y = `F0`, group = Conditions, color = Conditions)) + 
  ...
  labs(x = "", y = "Surface microalgal biomass (F0)", 
       title = "", 
...
ggplot(df_heatmap_diff_pv_long, aes(x = Condition, y = factor(Variable, levels = rev(unique(Variable))), fill = Categorie)) +
  ...
  annotate("text", 
           x = 2, y = 7.3, 
           label = "Sediment-associated biofilms", 
           size = 10, 
           colour = "black")+
  annotate("text", 
           x = 5, y = 7.3, 
           label = "Sediment-free biofilms", 
           size = 10, 
           colour = "black")+
...
p1 <- ggplot() +
  ...
  labs(title = "")+
  ylab("Mean de-epoxidation state (%)")+
...
# Group name
richesse_spe18_long <- richesse_spe18_long %>%
  mutate(Groups = recode(Groups,
                        "SB"  = "SAB",
                        "WSB" = "SFB"))

# Colors settings
colors_groups <- c("SAB" = "#CC6677", "SFB" = "#81B29A")

# Stat comparisons 
my_comparisons <- list(c("SFB", "SAB"))
...
ggplot(richesse_spe18_long, aes(x = Groups, y = Valeur, fill = Groups, color = Groups)) +
  ...
  scale_color_manual(values = c("SAB" = "black", "SFB" = "black")) + 
...
barplot_pigment2 <- ggplot(data_perc_wsb, aes(x = Conditions, y = Percentage, fill = Pigments)) +
  ...
  scale_y_continuous(labels = scales::percent_format()) + 
  theme_test() +
  labs(title = "Sediment-free biofilms", x = "", y = "Relative Abundance (%)") +
  theme(axis.text.x = element_blank())+
  scale_fill_manual(values = colors_barplot_pigments)+
...
# Treatment rename
df_pigment_perc$Treatment <- gsub("WSB", "SFB", df_pigment_perc$Treatment)
df_pigment_perc$Treatment <- gsub("SB",  "SAB", df_pigment_perc$Treatment)

df_pigment_perc$Conditions <- gsub("WSB", "SFB", df_pigment_perc$Conditions)
df_pigment_perc$Conditions <- gsub("SB",  "SAB", df_pigment_perc$Conditions)

df_pigment_perc$Groups <- gsub("WSB", "SFB", df_pigment_perc$Groups)
df_pigment_perc$Groups <- gsub("SB",  "SAB", df_pigment_perc$Groups)
...
#Table
df_pigment_perc_sb <- subset(df_pigment_perc, Groups == "SAB")
...
#Table
df_pigment_perc_wsb <- subset(df_pigment_perc, Groups == "SFB")
...
bca1 <- ggplot(res.bca.pigment_sb$ls,aes(x=res.bca.pigment_sb$ls[,1],y=res.bca.pigment_sb$ls[,2],col=fac))+
  ...
  annotate("text", 
           x = -4.4, y = 5, 
           label = "Sediment-associated biofilms (84%)", 
           size = 4, 
           colour = "black")
...
bca2 <- ggplot(res.bca.pigment_wsb$ls,aes(x=res.bca.pigment_wsb$ls[,1],y=res.bca.pigment_wsb$ls[,2],col=fac))+
  ...
  annotate("text", 
           x = -3, y = 5, 
           label = "Sediment-free biofilms (67%)", 
           size = 4, 
           colour = "black")
...
# Rename treatments
df_Pp_raw$Treatment <- gsub("WSB", "SFB", df_Pp_raw$Treatment)
df_Pp_raw$Treatment <- gsub("SB",  "SAB", df_Pp_raw$Treatment)
...
# Df structure
treatm_order <- c("DA SAB ⁽¹⁾ Before", "DA SAB ⁽¹⁾ After", "HL SAB Before", "HL SAB After", 
                  "DA SFB ⁽¹⁾ Before", "DA SFB ⁽¹⁾ After", "HL SFB Before", "HL SFB After",
                  "DA SAB ⁽²⁾ Before", "DA SAB ⁽²⁾ After", "HP SAB Before", "HP SAB After", 
                  "DA SFB ⁽²⁾ Before", "DA SFB ⁽²⁾ After", "HP SFB Before", "HP SFB After", 
                  "DA SAB ⁽³⁾ Before", "DA SAB ⁽³⁾ After", "PL SAB Before", "PL SAB After", 
                  "DA SFB ⁽³⁾ Before", "DA SFB ⁽³⁾ After", "PL SFB Before", "PL SFB After")
...
# Rename treatments
gdf_pigment_concentration$Treatment <- gsub("WSB", "SFB", gdf_pigment_concentration$Treatment)
gdf_pigment_concentration$Treatment <- gsub("SB",  "SAB", gdf_pigment_concentration$Treatment)

# Df structure
treatm_order <- c("DA SAB", "HL SAB", "HP SAB", "PL SAB", "DA SFB", "HL SFB", "HP SFB", "PL SFB")
